...
struct BNode
{
	...
	bool sonsMinimum() { int tmp = (int)(M / 2.0 + 0.5); return nsons <= tmp; }
...
void BNode<T>::moveToSubtreeFromBrother(int subTree, int subTreeBrother) //move one value and one subtree from subTreeBrother to subTree 
{
		...
		//larger value from subTreeBrother should be moved to father, and value from father should be moved to subTree
	{
		...
		holeNode->Son[0] = brother->Son[brother->nsons - 1]; //the last son of the brother is moved to the hole.														 //and number of sons in father is not changed.
	}
	else
		//smallest value from subTreeBrother should be moved to father
	{
		...
		holeNode->Son[holeNode->nsons] = ChildToSon; //the last son of the brother is moved to the hole.												 //and number of sons in father is not changed.
	}
	holeNode->nsons++; 
	holeNode->nkeys++;
	brother->nsons--; 
...
public:
	...
	T* search(BNode<T> *p, T x);//search a value and return it's pointer
	
	/**/void printBetween(T x, T y, BNode<T> *t); //prints all the keys between x and y
	/**/void printBetween(T x, T y) { printBetween(x, y, root); }
};

/**/template <typename T>
void BTree<T>::printBetween(T x, T y, BNode<T> *t)
{//print all the keys between x and y
	for (int i = 0; i < t->nsons; i++)
	{
		if (t->Son[i] != NULL)
		{
			printBetween(x, y, t->Son[i]);
		}
		if (i != t->nsons - 1)
			if (t->Key[i] >= x && t->Key[i] <= y)
				cout << t->Key[i];
	}
}
...
{//search a value and return it's pointer
	BNode<T> *v = searchInNode(x);
	if (v)
		return v;
	T *temp = p->Son;
	if (temp)
...
{
	for (int i = 0; i < t->nsons; i++)
	{//if the pointer isn't NULL so delete it
		if (t->Son[i])
			deleteSubTree(t->Son[i]);
	}
	for (int i = 0; i < t->nkeys; i++)
	{//delete the keys in the array
		deleteVal(t->Key[i]);
	}
}
...
{
	for (int i = 0; i < t->nsons; i++)
	{//if the pointer isn't NULL so print it
		if (t->Son[i] != NULL)
		{
